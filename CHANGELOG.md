...
- **Switched the security-scan runner from Semgrep to Opengrep (closes #111)** — [Opengrep](https://github.com/opengrep/opengrep) is an open, login-free fork of Semgrep with the same rule format and a semgrep-compatible CLI. The decisive difference: it ships as a **single standalone binary per platform** on GitHub releases with **no account, token, or telemetry**, so pi-lens can now **auto-install it on demand** via the existing `github` single-binary strategy — where Semgrep was deliberately *never* auto-installed (it needed pipx/brew + optional login). The runner (`opengrep`, replacing `semgrep`) auto-installs when elected, then runs `opengrep scan --json --timeout 5 --config <cfg> <file>`; note **no `--metrics=off`** (Opengrep has no metrics flag — it never phones home — so passing it would error). User surface is a clean rename with **no semgrep aliases** (per maintainer decision): flag `--lens-opengrep[-config]`, command `/lens-opengrep`, persisted `.pi-lens/opengrep.json`. Local rule-file discovery prefers `.opengrep.yml`/`.opengrep.yaml` but **also detects legacy `.semgrep.yml`/`.semgrep.yaml`**, since Opengrep consumes that rule format natively (a rule-file convention, not pi-lens branding). Wired via `withOpengrepGroup` (the dynamic opt-in dispatch group, sibling to `withSpotbugsGroup`) and registered in `TOOL_EXECUTION_POLICY` (config-first, auto-install) + `GITHUB_TOOLS`. **Validated end-to-end on the dev box**: opengrep 1.22.0 auto-installs and runs. Guards: `opengrep-config.test.ts` (resolution: local/flag/persisted/disabled + `.semgrep.yml` compat + arg normalization), `opengrep-parser.test.ts` (severity/semantic/defect-class mapping + high-signal-security promotion + path fallback + MAX_DIAGNOSTICS cap), and the github install-contract/full-matrix guards. _(FindSecBugs/PMD remain noted JVM follow-ups; unrelated.)_
