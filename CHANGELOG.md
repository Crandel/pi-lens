...
- **`lens_diagnostics mode=full` now surfaces jscpd (copy-paste) and madge (circular deps) findings** — previously only knip crossed from the heavyweight analyzers into the diagnostic surface; jscpd/madge findings only reached the agent via next-turn context injection. Two new runner-adapters (`project-diagnostics/runner-adapters/{jscpd,madge}.ts`, mirroring `knip.ts`) map their output to per-file `ProjectDiagnostic`s: a jscpd clone emits a diagnostic on **both** ends (each naming the other span), a madge cycle emits one on **each** participating file (rendering the whole cycle). **Cache-only, never re-launched:** jscpd is read from the session-start result cache; madge's turn-end run (which was previously computed and discarded) is now persisted into the project-diagnostics delta report — so `mode=full` consumes already-computed results and can't spawn a competing scan or contend with the background analyzers. Included when `refreshRunners` is `cached`/`cheap`/`all`. Note: madge coverage is scoped to files touched during the session (its turn-end cadence), while jscpd is whole-project (its session-start cadence).
